// Effet de lignes de code animées pour la transition entre les sections Formation et Compétences
document.addEventListener('DOMContentLoaded', function() {
  console.log('Script de transition Formation-Compétences chargé');
  
  // Création du conteneur pour l'effet de transition
  const transitionContainer = document.createElement('div');
  transitionContainer.className = 'code-transition education-skills-transition';
  ...
  revealContainer.className = 'reveal-transition education-skills-reveal';
  document.body.appendChild(revealContainer);
  
  // Création des éléments de révélation
  createRevealElements(revealContainer);
  
  // Lignes de code à afficher (extraits de code orientés compétences)
  const codeSnippets = [
    'const skills = {',
    '  frontend: ["HTML", "CSS", "JavaScript", "React"],',
    '  backend: ["Ruby", "Rails", "Node.js"],',
    ...
    '',
    '.skill-card {',
    '  background: linear-gradient(135deg, #2b3f8a, #4a5db2);',
    '  color: white;',
    '  border-radius: 8px;',
    '  padding: 15px;',
    '}',
    ...
    '  if (years < 1) return skillLevels.beginner;',
    '  if (years < 3) return skillLevels.intermediate;',
    '  if (years < 5) return skillLevels.advanced;',
  ...
  window.addEventListener('scroll', function() {
    ...
    const totalDistance = skillsTop - educationTop;
    
    // Définir la zone de transition (entre les deux sections)
    const transitionStart = skillsTop - 300; // Commencer 300px avant la section Compétences
    const transitionEnd = skillsTop - 50; // Terminer 50px avant la section Compétences
    
    console.log(`Scroll: ${scrollPosition}, Start: ${transitionStart}, End: ${transitionEnd}, Skills: ${skillsTop}`);
    ...
    if (scrollPosition >= transitionStart && scrollPosition <= transitionEnd) {
      ...
      console.log('Progression code Formation-Compétences:', progress);
...
function createRevealElements(container) {
  // Types d'éléments à révéler (spécifiques aux compétences)
  const elementTypes = [
    'circle', 'square', 'triangle', 'diamond', 'hexagon',
    'code', 'tag', 'database', 'server', 'browser',
