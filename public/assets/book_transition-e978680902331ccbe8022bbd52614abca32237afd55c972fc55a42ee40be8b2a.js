...
document.addEventListener('DOMContentLoaded', function() {
  ...
  bookCover.style.height = '75%';
  bookCover.style.backgroundColor = '#2b3f8a'; // Bleu foncé pour contraster avec le fond rouge
  bookCover.style.border = '5px solid white';
  bookCover.innerHTML = `
  ...
  // Ajout d'un élément de fond pour améliorer la visibilité
  ...
  bookTransitionContainer.appendChild(backgroundOverlay);
  ...
  window.addEventListener('scroll', function() {
    ...
    if (scrollPosition >= transitionStart && scrollPosition <= transitionEnd) {
      ...
      // Rendre le conteneur de livre visible plus rapidement
      bookTransitionContainer.style.opacity = Math.min(1, progress * 3);
      ...
      // Animer le fond
      backgroundOverlay.style.opacity = Math.min(1, progress * 2.5);
...
function createBookPage(container, content, index) {
  ...
  // Augmenter la taille des pages pour une meilleure visibilité
  page.style.width = '50%';
  page.style.height = '75%';
