...
document.addEventListener('DOMContentLoaded', function() {
  console.log('DOMContentLoaded - initialisation directe des cartes');
  initializeSkillCardsDirectly();
});

document.addEventListener('turbo:load', function() {
  console.log('turbo:load - initialisation directe des cartes');
  initializeSkillCardsDirectly();
});

// Exécuter immédiatement
console.log('Exécution immédiate - initialisation directe des cartes');
setTimeout(initializeSkillCardsDirectly, 0);

function initializeSkillCardsDirectly() {
  console.log('Initialisation directe des cartes de compétences...');
  ...
  if (cards.length === 0) {
    console.log('Aucune carte trouvée, réessai dans 500ms');
  ...
  cards.forEach((card, index) => {
    ...
    card.addEventListener('click', function(e) {
      console.log(`Clic sur la carte ${index + 1}`);
      e.preventDefault();
      e.stopPropagation();
      
      // Si cette carte est déjà active, la fermer
      if (activeCard === this) {
        console.log('Fermeture de la carte active');
        closeCard(this);
        return;
      }
      ...
      if (activeCard) {
        console.log('Fermeture de la carte précédente');
  ...
  overlay.addEventListener('click', function() {
    if (activeCard) {
      console.log('Clic sur overlay - fermeture de la carte active');
      closeCard(activeCard);
    }
  });
  ...
  console.log('Initialisation directe des cartes terminée');
};
