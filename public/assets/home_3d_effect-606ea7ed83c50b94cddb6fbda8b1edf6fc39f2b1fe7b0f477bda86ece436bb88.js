...
document.addEventListener('turbo:load', function() {
  console.log('turbo:load fired - initialisation des effets 3D pour la section home');
  initializeHome3DEffect();
});

document.addEventListener('DOMContentLoaded', function() {
  console.log('DOMContentLoaded fired - initialisation des effets 3D pour la section home');
  initializeHome3DEffect();
});
...
function initializeHome3DEffect() {
  console.log('Initialisation des effets 3D pour la section home...');
  
  // Sélectionner le conteneur animé
  const animatedContainer = document.querySelector('.animated-container');
  
  if (!animatedContainer) {
    console.log('Le conteneur animé n\'a pas été trouvé');
    return;
  }
  
  console.log('Conteneur animé trouvé, ajout des écouteurs d\'événements');
  ...
  animatedContainer.addEventListener('mousemove', function(e) {
    handleMouseMove(e, animatedContainer, title, text, buttons, bracketTopLeft, bracketBottomRight);
  });
  
  // Réinitialiser l'effet 3D quand la souris quitte le conteneur
  animatedContainer.addEventListener('mouseleave', function() {
    resetContainerTransform(animatedContainer, title, text, buttons, bracketTopLeft, bracketBottomRight);
  });
  
  console.log('Initialisation des effets 3D pour la section home terminée');
}

// Fonction pour gérer l'effet 3D au survol
function handleMouseMove(e, container, title, text, buttons, bracketTopLeft, bracketBottomRight) {
  ...
  if (bracketTopLeft) {
    ...
    bracketTopLeft.style.boxShadow = `
      ${-x * 12}px ${-y * 12}px 15px rgba(0, 0, 0, 0.3)
    `;
  }
  ...
  if (bracketBottomRight) {
    ...
    bracketBottomRight.style.boxShadow = `
      ${x * 12}px ${y * 12}px 15px rgba(0, 0, 0, 0.3)
    `;
  }
}
...
function resetContainerTransform(container, title, text, buttons, bracketTopLeft, bracketBottomRight) {
