...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const animateTimelinePoints = () => {
    ...
    companyCards.forEach(card => {
      // Observer chaque carte pour déclencher l'animation du point
      const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            // Ajouter la classe pour animer le point de la timeline
            setTimeout(() => {
              card.classList.add('animate-timeline-point');
            }, 800); // Délai après l'animation de la carte
          } else {
            // Retirer la classe si la carte n'est plus visible
            if (scrollDirection === 'up') {
              card.classList.remove('animate-timeline-point');
            }
          }
        });
      }, {
