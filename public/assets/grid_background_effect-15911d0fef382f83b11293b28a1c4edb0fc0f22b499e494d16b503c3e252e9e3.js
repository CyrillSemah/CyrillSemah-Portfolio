...
document.addEventListener('DOMContentLoaded', function() {
  ...
  setTimeout(initializeGridBackgroundEffect, 100);
});

// Vérifier périodiquement si la grille est disponible
const gridCheckInterval = setInterval(function() {
  if (!gridInitialized && document.querySelector('.grid-background')) {
    console.log('Grille d\'arrière-plan détectée par l\'intervalle');
    initializeGridBackgroundEffect();
...
function initializeGridBackgroundEffect() {
  ...
  if (!gridBackground) {
    console.log('La grille d\'arrière-plan n\'a pas été trouvée, réessai plus tard');
    return;
  }
